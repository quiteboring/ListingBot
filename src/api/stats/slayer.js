import { xp_tables } from '../constants/xp_tables.js';

const getSlayerLevel = (profile, slayer) => {
  ...
  progress =
    level >= maxLevel
      ? 0
      : Math.max(0, Math.min(experience / xpForNext, 1));
  ...
  for (let i = 0; i < Object.keys(slayers).length; i++) {
    if (Object.keys(slayers)[i].startsWith('boss_kills_tier_')) {
      ...
      kills[
        (
          Number(
            Object.keys(slayers)[i].charAt(
              Object.keys(slayers)[i].length - 1,
            ),
          ) + 1
        ).toString()
...
export const getSlayer = (profile) => {
  try {
    return {
    ...
    throw new Error('Unable to find slayer data.');
  }
};
